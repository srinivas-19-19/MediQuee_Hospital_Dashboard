...
export function Dashboard() {
  ...
  const quickActions = [
    { icon: Calendar, label: 'Appts', path: '/appointments', bg: 'bg-blue-50', text: 'text-primary' },
    { icon: IndianRupee, label: 'Payout', path: '/payouts', bg: 'bg-teal-50', text: 'text-teal-600' },
    { icon: Stethoscope, label: 'Doctors', path: '/profile/staff/doctors', bg: 'bg-indigo-50', text: 'text-indigo-600' },
  ];
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <motion.div 
          ...
          className="bg-white rounded-2xl p-4 shadow-[0_1px_2px_rgba(0,0,0,0.02)] border border-gray-100/60 flex flex-col justify-between gap-3 cursor-pointer hover:border-blue-200 hover:shadow-md transition-all active:scale-[0.98] group"
          title="View Today's OPs"
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="p-1.5 bg-blue-50 text-primary rounded-lg group-hover:bg-blue-100/80 transition-colors">
        ...
        <motion.div 
          variants={item}
          className="bg-white rounded-2xl p-3 sm:p-4 shadow-[0_1px_2px_rgba(0,0,0,0.02)] border border-gray-100/60 flex flex-col justify-between hover:border-gray-200 transition-all group"
        >
          <div className="grid grid-cols-2 divide-x divide-gray-100 h-full">
            ...
            <div 
              ...
              className="flex flex-col justify-between pr-2.5 sm:pr-3 cursor-pointer group/pending hover:bg-amber-50/40 -my-1 -ml-1 py-1 pl-1 rounded-xl transition-all active:scale-[0.97]"
              title="View Pending OPs"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <div className="p-1 bg-amber-50 text-amber-600 rounded-lg group-hover/pending:bg-amber-100/80 transition-colors">
                    <Clock className="w-3.5 h-3.5" />
                  </div>
                  <span className="text-[11px] sm:text-[12px] font-medium text-[#667085] truncate">Pending</span>
                </div>
                <ChevronRight className="w-3 h-3 text-gray-300 group-hover/pending:text-amber-600 group-hover/pending:translate-x-0.5 transition-all hidden sm:block" />
              </div>
              <div className="flex flex-col min-h-[32px] justify-center mt-1">
                {isLoading && !overview ? (
                  <Skeleton className="h-7 w-10" />
                ) : (
                  <span className="text-[20px] sm:text-[22px] font-semibold text-[#172033] tracking-tight">
            ...
            <div 
              ...
              className="flex flex-col justify-between pl-2.5 sm:pl-3 cursor-pointer group/completed hover:bg-emerald-50/40 -my-1 -mr-1 py-1 pr-1 rounded-xl transition-all active:scale-[0.97]"
              title="View Completed OPs"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <div className="p-1 bg-emerald-50 text-emerald-600 rounded-lg group-hover/completed:bg-emerald-100/80 transition-colors">
                    <CheckCircle2 className="w-3.5 h-3.5" />
                  </div>
                  <span className="text-[11px] sm:text-[12px] font-medium text-[#667085] truncate">Completed</span>
                </div>
                <ChevronRight className="w-3 h-3 text-gray-300 group-hover/completed:text-emerald-600 group-hover/completed:translate-x-0.5 transition-all hidden sm:block" />
              </div>
              <div className="flex flex-col min-h-[32px] justify-center mt-1">
                {isLoading && !overview ? (
                  <Skeleton className="h-7 w-10" />
        ...
        <motion.div 
          ...
          className="bg-white rounded-2xl p-4 shadow-[0_1px_2px_rgba(0,0,0,0.02)] border border-gray-100/60 flex flex-col justify-between gap-3 cursor-pointer hover:border-purple-200 hover:shadow-md transition-all active:scale-[0.98] group"
          title="View Lab Tests"
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="p-1.5 bg-purple-50 text-info rounded-lg group-hover:bg-purple-100/80 transition-colors">
                <FileText className="w-4 h-4" />
              </div>
              <span className="text-[13px] font-medium text-[#667085]">Lab Tests</span>
            </div>
            <ChevronRight className="w-3.5 h-3.5 text-gray-300 group-hover:text-info group-hover:translate-x-0.5 transition-all" />
          </div>
          <div className="flex flex-col min-h-[32px] justify-center">
            {isLoading && !overview ? (
              <Skeleton className="h-7 w-16" />
        ...
        <motion.div 
          ...
          className="bg-white rounded-2xl p-4 shadow-[0_1px_2px_rgba(0,0,0,0.02)] border border-gray-100/60 flex flex-col justify-between gap-3 cursor-pointer hover:border-teal-200 hover:shadow-md transition-all active:scale-[0.98] group"
          title="View Revenue & Payouts"
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="p-1.5 bg-teal-50 text-teal rounded-lg group-hover:bg-teal-100/80 transition-colors">
                <IndianRupee className="w-4 h-4" />
              </div>
              <span className="text-[13px] font-medium text-[#667085]">Revenue Today</span>
            </div>
            <ChevronRight className="w-3.5 h-3.5 text-gray-300 group-hover:text-teal group-hover:translate-x-0.5 transition-all" />
          </div>
          <div className="flex flex-col min-h-[32px] justify-center">
            {isLoading && !overview ? (
      ...
      <motion.div variants={item} className="bg-white rounded-2xl border border-gray-100/50 shadow-[0_1px_2px_rgba(0,0,0,0.02)] p-4 flex flex-col gap-4">
        <div className="flex items-center justify-between relative">
          <div className="flex flex-col">
            ...
            <span className="text-[12px] font-medium text-[#667085]">{timeframeConfig[timeframe].subtitle}</span>
          </div>
          ...
          <div className="relative" ref={dropdownRef}>
            <button 
              ...
              className="flex items-center gap-1.5 text-[12px] font-semibold text-[#172033] bg-[#F7F8FA] hover:bg-gray-100 active:bg-gray-200 px-3 py-1.5 rounded-xl border border-gray-200/60 shadow-sm transition-all"
            ...
            <AnimatePresence>
              {isDropdownOpen && (
                <motion.div
                  ...
                  className="absolute right-0 mt-1.5 w-36 bg-white rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.12)] border border-gray-100 py-1 z-30 overflow-hidden"
                >
                  {(['today', 'week', 'month', 'year'] as const).map((t) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-[13px] flex items-center justify-between transition-colors ${
                        timeframe === t 
                          ? 'bg-blue-50/70 text-primary font-bold' 
                          : 'text-[#172033] hover:bg-gray-50 font-medium'
                      }`}
                    >
                      <span>{timeframeConfig[t].label}</span>
                      {timeframe === t && <Check className="w-3.5 h-3.5 text-primary" />}
                    </button>
                  ))}
                </motion.div>
              )}
        ...
        <div className="grid grid-cols-3 gap-3 px-1">
          {quickActions.map((action, idx) => (
            <button 
              ...
              className="flex flex-col items-center justify-center gap-2 py-3.5 px-2 bg-white rounded-2xl border border-gray-100/70 shadow-[0_1px_3px_rgba(0,0,0,0.03)] hover:border-blue-200 hover:shadow-md active:scale-95 transition-all group"
            ...
            >
              ...
              <span className="text-[12px] font-semibold text-[#172033]">{action.label}</span>
            </button>
          ))}
        </div>
      </motion.div>
          ...
          {isLoading && !overview ? (
            ...
            activeAppointments.map((appt) => (
              <div 
                ...
                className="flex items-start gap-3 p-4 border-b border-gray-50 last:border-0 interactive-element active:bg-gray-50/50 transition-colors cursor-pointer hover:bg-gray-50/40"
              >
                ...
                <div className="flex flex-col flex-1">
                  <div className="flex justify-between items-start">
                    ...
                    <button 
                      ...
                      className="p-1 -mt-1 -mr-1 text-[#98A2B3] hover:text-[#172033] active:scale-95"
                    >
